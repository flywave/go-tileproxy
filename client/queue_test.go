...
func TestQueue(t *testing.T) {
	...
	q, err := NewQueue(10, 100000)
	if err != nil {
		panic(err)
	}
	put := func() {
		t := time.Duration(rng.Intn(50)) * time.Microsecond
		uri := server.URL + "/delay?t=" + t.String()
	...
	for i := 0; i < 3000; i++ {
		put()
		q.storage.AddFuture(newFuture([]byte("error request")))
	}
	c := crawler.NewCollector()
	c.OnRequest(func(req *crawler.Request) {
		atomic.AddUint32(&requests, 1)
	})
	c.OnResponse(func(resp *crawler.Response) {
	...
	c.OnError(func(resp *crawler.Response, err error) {
		atomic.AddUint32(&failure, 1)
	})
	err = q.Run(c)
	if err != nil {
